...
st.caption("정밀 시세 분석 및 3초 전략 판단 시스템 (v4.2)")
...
try:
    ...
    if not df.empty:
        ...
        if not f_df.empty:
            # ---------------------------------------------------------
            # 1. 상단 핵심 지표 (오류 수정: 필터 내 진짜 최저가 추출)
            # ---------------------------------------------------------
            ...
            with m_col1:
                if not amber_data.empty:
                    # [수정 포인트] 선택된 날짜/필터 내에서 엠버의 절대 최저가 검색
            ...
            # 신호등 스타일 함수 (빨강: 경쟁사 저가 공세 / 초록: 엠버 우세)
            def color_signal(val):
                if pd.isna(val) or amber_data.empty: return ''
                # 각 날짜별 엠버 최저가를 기준으로 실시간 비교
            ...
            with col_b:
                st.subheader("📊 호텔별 요금 범위 (Box Plot)")
        ...
        st.error("구글 시트 데이터 로드 실패. Collector.py를 실행해 주세요.")
