...
try:
    ...
    if not df.empty:
        ...
        if not amber_df.empty and not comp_df.empty:
            ...
            글로벌 채널 전략 분석: '아고다'와 '트립닷컴'의 가격을 별도로 추출하여, 국내 채널(트립비토즈 등)과의 가격 차이(Rate Parity)를 분석하라.

            역전 현상 포착: 만약 글로벌 채널 가격이 국내 채널보다 낮다면, 이는 점유율 50%를 끌어올릴 기회인지 혹은 수익성을 해치는 위협인지 판단하라.

            인바운드 타겟팅: 트립닷컴의 가격 경쟁력이 높다면 중화권/글로벌 고객 유치를 위한 공격적 가격 설정을 제안하고, 아고다의 가격이 높다면 리워드 프로그램(Genius 등) 활용 방안을 제시하라.
